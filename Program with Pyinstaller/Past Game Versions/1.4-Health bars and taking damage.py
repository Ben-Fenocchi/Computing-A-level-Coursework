#NEw in this version
#health bars, we got health bars now, taking damage also from the lava
#new cant collide list, if a letter in the game map is in the cant collide list then it wont be added to the list of tile rects which check if the player is colliding with them,i.e he doesnt walk on water now, he sinks
#problem when adding tiles to damage block list in that it kept doing it so list got longer and longer and going through list to check for collisions lagged everything out. To fix this have variable oneTimeDamageList that will let it be added to array only once
#added walls to the left and right so player cant walk off the edge of the maps
#Lava doesnt do damage if you dont move/dont move the mouse-this is a problem.... or hardmode?
import pygame,sys
...
red = (255,0,0)#not fully red but i like this one
green = (0,255,0)
blue = (0,0,255)
white = (255,255,255)
black = (0,0,0)
...
########################################################
def drawHealthBar():
    pygame.draw.rect(win,(255,(playerHealth),0),(250,205,playerHealth,60))#health bar will go more red as player gets lower health        
    win.blit(healthbar,(200,200))
########################################################
    ...
    while game:
        ...
        for row in game_map:                                                                                   #                                        
            ...
            for tile in row:                                                                                   #
                ...
                if tile == "k":                                                                                #                  
                    win.blit(cobble, ((x-1) * TILE_SIZEX, y * TILE_SIZEY))                                         #        Drawing the game level                                                 
                if tile == "l":                                                                                #                  
                    win.blit(lavaSurface, ((x-1) * TILE_SIZEX, y * TILE_SIZEY))                                    #
                    if oneTimeDamageList:                                                                      # 
...
################################################################                                               #                    
                ...
                x += 1
            y += 1
        oneTimeDamageList = False # so it only adds the damaging blocks to the array once, so it doesnt have an infinitely filling up array
